...
docker run --name redis_server -p 6379:6379 -d -v $(pwd)/volumn/redis_db:/data/db -v $(pwd)/log/redis_log:/data/log -v $(pwd)/conf/redis.conf:/usr/local/etc/redis/redis.conf redis:kytt
